...
# Forçar modo totalmente não interativo
export DEBIAN_FRONTEND=noninteractive
export DEBIAN_PRIORITY=critical

# Mata qualquer processo whiptail/dialog que possa ter ficado (segurança)
sudo pkill -9 whiptail 2>/dev/null || true
sudo pkill -9 dialog 2>/dev/null || true
...
# 0.1 Corrigir dpkg se necessário e remover travas
...
# 3. Instalar pacotes essenciais (pré-configuração robusta para evitar EULA)
echo -e "${VERDE}📦 Instalando pacotes básicos e apps solicitados...${SEM_COR}"

# Aceitar EULA das fontes Microsoft de todas as formas possíveis
...
# Instalar as fontes sem qualquer interação (usando apt-get com opções extras)
sudo apt-get install -y -qq \
    -o Dpkg::Options::="--force-confdef" \
    -o Dpkg::Options::="--force-confold" \
    ttf-mscorefonts-installer

# Instalar os demais pacotes (ubuntu-restricted-extras não pedirá as fontes novamente)
sudo apt install -y curl wget git zsh flatpak ubuntu-restricted-extras qbittorrent
