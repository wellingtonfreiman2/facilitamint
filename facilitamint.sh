...
# 4. Instalar Nerd Fonts (JetBrains Mono - Link Direto de Release)
echo -e "${VERDE}🔡 Instalando Nerd Fonts...${SEM_COR}"
mkdir -p ~/.local/share/fonts
rm -f jb_mono.zip # Remove arquivo corrompido de tentativas anteriores
curl -L "https://github.com" -o jb_mono.zip
unzip -o jb_mono.zip -d ~/.local/share/fonts
rm jb_mono.zip
fc-cache -f -v

# 5. Instalar Oh My Zsh + Plugins
